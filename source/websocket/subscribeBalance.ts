...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    const { wallet } = SubscribeBalanceSchema.parse(body);
    
    console.log(`Subscribing ${connectionId} to balance updates for ${wallet}`);
    ...
    const confirmationMessage = {
      ...
      wallet,
      message: `Subscribed to balance updates for ${wallet}`,
    };
    ...
    if (error instanceof z.ZodError) {
      errorMessage = "Invalid request: wallet is required";
    }
